...
{   
    ...
    // returns the coefficients of the new polynomial the evaluation at the challenge (to put on the transcript)
    {
        ...
        let m_e = self.sum(ctx, polynomial_even);
        let m_o = self.sum(ctx, polynomial_odd);

        // proof that m_e + m_o == target
        self.sum_check(ctx, vec![m_o, m_e], target);
        ...
        let polynomial_new_tail = 
                polynomial_even.iter()
                                .zip(polynomial_odd)
                                .map(|(a, b)| self.scalar_mul_and_add_no_carry(ctx, b.to_owned(), a.to_owned().to_owned(), challenge));

        polynomial_new.extend(polynomial_new_tail);

        let target_new = self.field_chip.add_no_carry(ctx, a, b);
...
//        mut transcript: impl TranscriptWrite<GA>,
    ) -> Result<(), Error> 
    {
        ...
        while polynomial.len() > 1{
            ...
            let next_challenge = challenges.iter().max_by_keynext()
